...
// 582小板
static lwns_addr_t dst_addr = {{0xba, 0x5d, 0x85, 0x26, 0x3b, 0x38}}; //目标节点地址，测试时，请根据电路板芯片MAC地址不同进行修改。修改为接收方的MAC地址，请勿使用自己的MAC地址

static lwns_ruc_controller ruc; //声明可靠单播控制结构体

static uint8_t TX_DATA[8] =
    {0xff, 0xc0, 0x68, 0x61, 0x00, 0x00, 0xff, 0xc0};
...
uint16_t lwns_ruc_ProcessEvent(uint8_t task_id, uint16_t events)
{
    ...
    {
        uint8_t temp = (uint8_t)GPIOA_ReadPortPin(GPIO_Pin_4);
        printf("pa4:%d\n",temp);
        TX_DATA[4] = temp == 0 ? 1 : 0;
